...
public class Spill {
	...
	private void delutKort() {
		...
		for(int i = 0; i < ANTALL_KORT_START; i++) {
			...
			Kort temp_kortNord = bord.taOversteFraBunke();
			this.nord.leggTilKort(temp_kortNord);
			
			Kort temp_kortSyd = bord.taOversteFraBunke();
			this.syd.leggTilKort(temp_kortSyd);
		}
		
		// TODO - END
		
		
		// ______________________________________________________ DONE ___________________________________________
	}
	...
	public Handling nesteHandling(ISpiller spiller) {
		...
		if (spiller.getAntallTrekk() > 0) {
			handling = new Handling(HandlingsType.TREKK, null);      
		}
		else if (leggnedKort(spiller, null)) {
			handling = new Handling(HandlingsType.LEGGNED, null);      
		}
		else {
